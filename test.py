...
from metrics import batch_iou, mean_iou, iou_score ,ppv,sensitivity, accuracy, get_DC, dice_coef, dice_coeff, mean_iou, specificity
import losses
...
def parse_args():
    ...
    parser.add_argument('--name', default='isic_Dilated_Unet_lr3e-4_512c_noaug_woDS',
                        help='model name')
...
def main():
    ...
    #model = model.__dict__['unet'](args)
    model = torch.nn.DataParallel(model, device_ids=device_ids)
    #model = model.cuda()
    model.to("cuda:0")

    # Data loading code
    #img_paths = glob(r'E:\pwd\Brats\BraTS2Dpreprocessing-master\testImage\*')
    #mask_paths = glob(r'E:\pwd\Brats\BraTS2Dpreprocessing-master\testMask\*')
    test_paths = '../ISIC/test.txt'
    ...
    model.load_state_dict(torch.load('models/%s/model-70epoch.pth' %args.name))
    model.eval()
    fh = open(test_paths, 'r')
    test_img = []
    test_mask = []
    for line in fh:
        line = line.rstrip()
        words = line.split()
        test_img.append(words[0])
        test_mask.append(words[1])
    test_dataset = Dataset(args, test_img, test_mask)
    ...
    if val_args.mode == "GetPicture":
            ...
            with torch.no_grad():
                for i, (input, target) in tqdm(enumerate(test_loader), total=len(test_loader)):
                    ...
                    for x in range(output.shape[0]):
                        #i = i + x
                        acc = accuracy(output[x], target[x])
                        accs.append(acc)
                        dice = get_DC(output[x], target[x])
                        dices.append(dice)
                        jac = iou_score(output[x], target[x])
                        jacs.append(jac)
                        iou = mean_iou(output[x], target[x])
                        ious.append(iou)
                        sen = sensitivity(output[x], target[x])
                        sens.append(sen)
                        spe = specificity(output[x], target[x])
                        specs.append(spe)
                        output[x] = torch.sigmoid(output[x])
                        output[x] = output[x] > 0.5
                        vutils.save_image(input[x], 'output/%s/' % args.name + '%d_orig.png' % i, padding=0)
                        vutils.save_image(output[x], 'output/%s/' %args.name + '%d.png' % i, normalize=True,
                                          padding=0)

                        vutils.save_image(target[x], 'output/%s/' %args.name + '%d_GT.png' % i, padding=0)
                    '''
        ...
        print('Jac: %.4f' % np.mean(jacs))
        print('Iou: %.4f' % np.mean(ious))
        print('Sen: %.4f' % np.mean(sens))
        print('Spe: %.4f' % np.mean(specs))
        print("Done!")
